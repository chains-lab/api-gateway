...
func UpdateOfficialByAdmin(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	res, err := ElectorCabAdminClient(r).UpdateOfficialByAdmin(signature, &electorcab.UpdateOfficialByAdminRequest{
		UserId: initiator.UserID.String(),
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating official by admin for user %s", initiator.UserID)
