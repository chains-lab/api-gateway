...
func UpdateOwnDegree(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	// Assuming the update logic is implemented in the ElectorCabUserClient
	job, err := ElectorCabUserClient(r).UpdateOwnDegree(signature, &electorcab.UpdateOwnDegreeRequest{
		Degree: degree,
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating own degree for user %s", initiator.UserID)
		renderer.RenderGRPCError(w, requestID, err)

		return
	}

	renderer.Render(w, responses.JobResume(job))
