...
func UpdateOwnBirthday(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating own birthday for user %s", initiator.UserID)
