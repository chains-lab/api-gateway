...
func UpdateOwnIncome(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	income := chi.URLParam(r, "income")

	job, err := ElectorCabUserClient(r).UpdateOwnIncome(signature, &electorcab.UpdateOwnIncomeRequest{
		Income: income,
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating own income for user %s", initiator.UserID)
