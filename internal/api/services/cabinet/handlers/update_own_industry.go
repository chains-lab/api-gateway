...
func UpdateOwnIndustry(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	job, err := ElectorCabUserClient(r).UpdateOwnIndustry(signature, &electorcab.UpdateOwnIndustryRequest{
		Industry: industry,
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating industry for user %s", initiator.UserID)
