...
func UpdateOwnNationality(w http.ResponseWriter, r *http.Request) {
	requestID := uuid.New()
	...
	nationality := chi.URLParam(r, "nationality")

	bio, err := ElectorCabUserClient(r).UpdateOwnNationality(signature, &electorcab.UpdateOwnNationalityRequest{
		Nationality: nationality,
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating own profile")
