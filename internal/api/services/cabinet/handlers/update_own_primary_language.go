...
func UpdateOwnPrimaryLanguage(w http.ResponseWriter, r *http.Request) {
	...
	bio, err := ElectorCabUserClient(r).UpdateOwnPrimaryLanguage(signature, &electorcab.UpdateOwnPrimaryLanguageRequest{
		PrimaryLanguage: language,
	})
	if err != nil {
		Log(r, requestID).WithError(err).Errorf("error updating own primary language for user %s", initiator.UserID)
